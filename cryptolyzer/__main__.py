...
from cryptolyzer.common.analyzer import ProtocolHandlerBase, AnalyzerBase
...
def get_handler_and_uris(parser, arguments):
    ...
    for analyzable_uri in uris:
        if analyzable_uri.scheme not in supported_schemes:
            unsupported_protocols.add(analyzable_uri.scheme)
    if unsupported_protocols:
        parser.error('unsupported protocol ({})'.format(', '.join(unsupported_protocols)))
