...
class AnalyzerPublicKeys(AnalyzerTlsBase):
    ...
    def _get_tls_public_key(self, server_messages):
            certificate_chain = []
            certificate_bytes = []
    ...
    def _is_cn_matches(self, cn, hostname):
        ...
        for index in range(-1, -len(hostname_parts) - 1, -1):
            if index < -len(cn_parts):
                ...
                return True
            elif cn_parts[index] != hostname_parts[index]:
                return False

        return True

    def _is_subject_matches(self, tls_public_key, hostname):
        cert = tls_public_key.certificate_chain[0]
        common_names = [attr.value for attr in cert.subject.get_attributes_for_oid(cryptography.x509.oid.NameOID.COMMON_NAME)]
        ...
        for name in common_names + subject_alternative_names:
            if self._is_cn_matches(name, hostname):
                return True
